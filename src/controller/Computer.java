...
//    public int[] Attack_core = {0, 2, 9, 81, 729, 6561};
    ...
    private int scoreRow(Point point, char data[][], int player) {
        int scoreRow = 0;
        int count = 1;
        int quanDich = 0;
        int space = 0;
...
//                j++;
    ...
    public Point StepCom(char data[][]) {
        ...
        int CoreMax = 0;
        for (point.x = 0; point.x < 20; point.x++) {
    ...
    public void ComputerGo(Point point, JButton btn[][]) {
        if (pointTemp != null) {
            btn[pointTemp.x][pointTemp.y].setBackground(Color.white);
        }
        btn[point.x][point.y].setIcon(new javax.swing.ImageIcon(getClass().getResource("/Image/O.jpg")));
